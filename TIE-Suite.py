...
class Player(Frame):
    ...
    def __init__(self, parent, title=None):
        ...
        runMenu.add_command(label="Exit", command= lambda: _quit())
        menubar.add_cascade(label="Run", menu=runMenu)

        #Create projMenu Menu List
        projMenu = Menu(menubar, tearoff = 0)
        projMenu.add_command(label = "Load Project", command = lambda: loadProj(parent))
        ...
        loadMenu.add_command(label = "Load Theme - Client", command = lambda: loadTheme(parent, "Client"))
        loadMenu.add_command(label = "Load Theme - Counselor", command = lambda: loadTheme(parent, "Counselor"))
        loadMenu.add_command(label = "Load Workbook", command = lambda: loadExcel(parent))
        loadMenu.add_command(label = "Load Video File", command = lambda: vidLoad(parent))
...
def vidLoad(master):
    ...
    fullname =  askopenfilename(initialdir = "./Video Files", title = "Choose a Video File",filetypes = (("all files","*.*"),("mp4 files","*.mp4")))
    if os.path.isfile(fullname):
        dirname  = os.path.dirname(fullname)
        filename = os.path.basename(fullname)
    vidFile = str(dirname + "/" + filename)
